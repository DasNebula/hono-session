...
const kv = new MockKV()
const getCookies = (response: Response, name = '__session'): {
  value: string,
  attrs: {
    [k: string]: string
  }[],
...
describe('Session KV Middleware', () => {
  ...
  app.use('/session/error/ttl-too-low', (c, next) => {
    return sessionStart({
      kvNamespace: kv.namespace,
      ttl: 1,
    })(c, next)
  })
  app.use('/session/error/ttl-too-high', (c, next) => {
    return sessionStart({
      kvNamespace: kv.namespace,
      ttl: 401 * 24 * 60 * 60,
    })(c, next)
  })
  app.use('/session/error/invalid-kv', (c, next) => {
    return sessionStart({
      kvNamespace: 'foo',
    })(c, next)
  })
  ...
  describe('sessionStart middleware', () => {
    ...
    it('Should return 500 if KV is invalid', async () => {
      const res = await app.request('/session/error/invalid-kv')
      expect(res.status).toBe(500)
      expect(await res.text()).toBe('Session datastore was not found.')
    })

    it('Should return 500 if TTL is too low', async () => {
      const res = await app.request('/session/error/ttl-too-low')
      expect(res.status).toBe(500)
      expect(await res.text()).toBe('Invalid TTL')
    })

    it('Should return 500 if TTL is too high', async () => {
      const res = await app.request('/session/error/ttl-too-high')
      expect(res.status).toBe(500)
      expect(await res.text()).toBe('Invalid TTL')
    })
  })
